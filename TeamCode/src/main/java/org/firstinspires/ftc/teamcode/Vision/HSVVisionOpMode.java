...
public class HSVVisionOpMode extends LinearOpMode {
    ...
    OpenCvWebcam webcam;
    HSVSampleDetectionPipeline pipeline;
    ...
    public void runOpMode() {
        ...
        pipeline = new HSVSampleDetectionPipeline();
        ...
        while (opModeInInit()) {
            ...
            ArrayList<HSVSampleDetectionPipeline.AnalyzedStone> detectedStones = pipeline.getDetectedStones();
            double angle;
            // Display the detected stones and their properties on telemetry
            for (int i = 0; i < detectedStones.size(); i++) {
                HSVSampleDetectionPipeline.AnalyzedStone stone = detectedStones.get(i);
                telemetry.addData("Stone " + i, "Color: %s, Angle: %.2f", stone.color, stone.angle);
            }

            HSVSampleDetectionPipeline.AnalyzedStone targetSample;
